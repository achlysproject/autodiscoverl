...
RELEASE_DIR  = $(CURDIR)/_build/default/rel/autodiscoverl
VERSION     := $(shell erl -noshell -eval 'io:format("~s", [case file:consult("$(CURDIR)/src/autodiscoverl.app.src") of {ok, [{_, _, Terms}]} -> proplists:get_value(vsn, Terms, "0.0.0"); _ -> "0.0.0" end]),erlang:halt().')
RELEASE_NAME = autodiscoverl-$(VERSION)
NAME_UPPER  := $(shell echo autodiscoverl | awk '{print toupper($$1)}')
...
compile:
	...
	$(PRE) \
            ...
            $(REBAR) compile \
        $(POST)
	$(PRE) cp -r $(CURDIR)/_build/default/lib/autodiscoverl/ebin $(CURDIR)
...
shell:
	...
	$(PRE) \
        ...
        erl     -pa `ls -d _build/default/lib/*/ebin` \
                ...
                -eval "begin application:load('autodiscoverl'), catch code:load_file('autodiscoverl') end" \
                +B
...
docker:
	$(PRE) docker build -t autodiscoverl ./ $(POST)
